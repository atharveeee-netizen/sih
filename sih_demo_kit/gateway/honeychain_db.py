...
def init_db():
    ...
    # 16. Ledger Events (local SHA-256 forward-linked hash chain -- see BlockchainLedgerFacade
    #     in blockchain_bridge.py and .spec/Schema.md). This table was previously referenced
    #     by the idx_ledger_batch index below (and inserted into by seed_honeychain_demo.py /
    #     honeychain_api.py) without ever being created here -- a fresh database would throw
    #     sqlite3.OperationalError on the index-creation line. Fixed by actually creating it.
    cursor.execute("""
    ...
    # 17. Chain Farmers -- maps an off-chain beekeeper_id to the demo wallet address
    # HoneyChainBlockchainBridge registered on-chain for them, and the on-chain farmerId
    # the contract assigned, so a beekeeper is only ever registered once.
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS chain_farmers (
        beekeeper_id TEXT PRIMARY KEY,
    ...
    # 18. Chain Transactions -- links an off-chain entity (a harvest or a batch) to the
    # real on-chain transaction that represents it (submitHarvest / approveHarvestAndMint),
    # so the gateway API can report a genuine tx_hash/block_number instead of inventing one.
    cursor.execute("""
    CREATE TABLE IF NOT EXISTS chain_transactions (
        id TEXT PRIMARY KEY,
        entity_type TEXT NOT NULL, -- HARVEST, BATCH
        entity_id TEXT NOT NULL,
        onchain_id INTEGER,        -- requestId for HARVEST, batchId for BATCH
        event_type TEXT NOT NULL,  -- HarvestSubmitted, BatchMinted, ...
    ...
    cursor.execute("CREATE INDEX IF NOT EXISTS idx_chain_tx_entity ON chain_transactions (entity_type, entity_id);")
