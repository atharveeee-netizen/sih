...
def record_harvest(h: HarvestCreate):
    ...
    # Real on-chain submitHarvest() call against HoneyChain.sol (local Hardhat node by
    # default -- see BLOCKCHAIN_RPC_URL / HONEYCHAIN_CONTRACT_ADDRESS / BLOCKCHAIN_PRIVATE_KEY
    # env vars). Registers the beekeeper's demo wallet as an on-chain farmer the first time,
    # then submits the harvest from that wallet. If no node is reachable this returns
    # chain_status: "offline" and the harvest record above is unaffected -- SQLite is always
    # the source of truth for the gateway itself; the chain call is additive, not blocking.
    chain_conn = get_db()
    chain_cursor = chain_conn.cursor()
    chain_cursor.execute("""
        SELECT bk.name AS bk_name, c.district AS district, bk.registration_no AS reg_no
        FROM beekeepers bk JOIN clusters c ON bk.cluster_id = c.id
        WHERE bk.id = ?;
    """, (h.beekeeper_id,))
    bk_row = chain_cursor.fetchone()
    chain_conn.close()

    chain_result = {"chain_status": "offline"}
    if bk_row:
        chain_result = blockchain_bridge.submit_harvest_onchain(
            beekeeper_id=h.beekeeper_id,
            beekeeper_name=bk_row["bk_name"],
            location=bk_row["district"],
            cooperative_id=bk_row["reg_no"],
            flora_source=h.floral_source,
            quantity_kg=h.quantity_kg,
        )
        if chain_result.get("chain_status") == "on_chain":
            chain_conn = get_db()
            chain_cursor = chain_conn.cursor()
            chain_cursor.execute("""
                INSERT INTO chain_transactions (id, entity_type, entity_id, onchain_id, event_type, tx_hash, block_number, contract_address, created_at)
                VALUES (?, 'HARVEST', ?, ?, 'HarvestSubmitted', ?, ?, ?, ?);
            """, (
                f"CTX-{uuid.uuid4().hex[:8].upper()}", harvest_id, chain_result.get("request_id"),
                chain_result.get("tx_hash"), chain_result.get("block_number"),
                chain_result.get("contract_address"), datetime.now(timezone.utc).isoformat(),
            ))
            chain_conn.commit()
            chain_conn.close()

    return {"harvest_id": harvest_id, "status": "RECORDED", "chain": chain_result}
...
def attach_packaging_lot(batch_id: str, pkg: PackageCreate):
    ...
    # Real on-chain approveHarvestAndMint() call -- packaging is the first point in the
    # gateway's pipeline where a real qrToken exists, which the contract requires atomically
    # with the mint. Needs: the on-chain requestId from this batch's original submitHarvest
    # (looked up from chain_transactions), and a quality_score/grade derived from this
    # batch's actual lab test data (not a hardcoded number). If either is missing --
    # no on-chain request, or no quality test done yet -- this is skipped and reported as
    # chain_status "skipped", rather than minting with invented inputs.
    chain_conn = get_db()
    chain_cursor = chain_conn.cursor()
    chain_cursor.execute("SELECT harvest_id FROM honey_batches WHERE id = ?;", (batch_id,))
    batch_row = chain_cursor.fetchone()
    chain_result = {"chain_status": "skipped", "reason": "batch not found"}
    if batch_row:
        chain_cursor.execute("""
            SELECT onchain_id FROM chain_transactions
            WHERE entity_type = 'HARVEST' AND entity_id = ? ORDER BY created_at DESC LIMIT 1;
        """, (batch_row["harvest_id"],))
        harvest_chain_row = chain_cursor.fetchone()
        chain_cursor.execute("""
            SELECT moisture_pct, hmf_mg_kg, adulteration_result, status FROM quality_tests
            WHERE batch_id = ? ORDER BY tested_at DESC LIMIT 1;
        """, (batch_id,))
        quality_row = chain_cursor.fetchone()
        chain_conn.close()

        if not harvest_chain_row or harvest_chain_row["onchain_id"] is None:
            chain_result = {"chain_status": "skipped", "reason": "no on-chain harvest request for this batch"}
        elif not quality_row:
            chain_result = {"chain_status": "skipped", "reason": "no quality test recorded yet"}
        else:
            score = 100.0
            if quality_row["moisture_pct"] > 20.0:
                score -= (quality_row["moisture_pct"] - 20.0) * 2
            if quality_row["hmf_mg_kg"] > 40.0:
                score -= (quality_row["hmf_mg_kg"] - 40.0)
            quality_score = max(0, min(100, round(score)))
            passed = quality_row["status"] == "PASS"
            grade = "FSSAI Grade A (Pure Authentic)" if (passed and quality_score >= 90) \
                else "FSSAI Grade B" if passed else "Flagged - Below Standard"

            chain_result = blockchain_bridge.approve_harvest_and_mint_onchain(
                request_id=harvest_chain_row["onchain_id"],
                quality_score=quality_score,
                grade=grade,
                qr_token=packages[0]["package_code"],
            )
            if chain_result.get("chain_status") == "on_chain":
                insert_conn = get_db()
                insert_conn.cursor().execute("""
                    INSERT INTO chain_transactions (id, entity_type, entity_id, onchain_id, event_type, tx_hash, block_number, contract_address, created_at)
                    VALUES (?, 'BATCH', ?, ?, 'BatchMinted', ?, ?, ?, ?);
                """, (
                    f"CTX-{uuid.uuid4().hex[:8].upper()}", batch_id, chain_result.get("batch_id"),
                    chain_result.get("tx_hash"), chain_result.get("block_number"),
                    chain_result.get("contract_address"), datetime.now(timezone.utc).isoformat(),
                ))
                insert_conn.commit()
