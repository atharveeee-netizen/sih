...
def init_database():
    ...
    try:
        ...
        pass
    try:
        cursor.execute("ALTER TABLE hives ADD COLUMN location TEXT DEFAULT 'Apiary Site';")
    except sqlite3.OperationalError:
        pass
    ...
    if cursor.fetchone()["count"] == 0:
        ...
        # honeychain_db.py's hives table enforces FOREIGN KEY(apiary_id) -> apiaries.id
        # and FOREIGN KEY(beekeeper_id) -> beekeepers.id (which itself references
        # clusters.id). The 100 demo hives below are hardcoded to APIARY-NILGIRIS-01 /
        # BK-TN-2026-001, so those rows -- and the cluster they chain up to -- must
        # exist first, or every insert fails with "FOREIGN KEY constraint failed" on
        # a fresh database. Seeded with INSERT OR IGNORE so this is safe to re-run
        # and does not clobber real data if these rows already exist.
        try:
            cursor.execute("""
            INSERT OR IGNORE INTO clusters (id, name, state, district, pincode, contact_officer, officer_phone, created_at)
            VALUES ('CLUSTER-1', 'Nilgiris Hub', 'Tamil Nadu', 'Nilgiris', '643001', 'R. Kumar', '9000000000', ?);
            """, (now_str,))
            cursor.execute("""
            INSERT OR IGNORE INTO beekeepers (id, cluster_id, name, registration_no, phone, aadhaar_masked, bank_linked, created_at)
            VALUES ('BK-TN-2026-001', 'CLUSTER-1', 'Murugan S', 'KVIC-REG-001', '9111111111', 'XXXX-1234', 1, ?);
            """, (now_str,))
            cursor.execute("""
            INSERT OR IGNORE INTO apiaries (id, beekeeper_id, name, latitude, longitude, elevation_m, primary_flora, total_boxes, created_at)
            VALUES ('APIARY-NILGIRIS-01', 'BK-TN-2026-001', 'Nilgiris Demo Apiary', 11.4916, 76.7337, 1800.0, 'Wild Forest Flora', 100, ?);
            """, (now_str,))
        except sqlite3.OperationalError:
            # honeychain_db.py hasn't created these tables yet (older schema) --
            # nothing to seed against, the plain hives table has no FK to satisfy.
            pass
        ...
        # apiary_id/beekeeper_id are now supplied explicitly (previously relied on
        # column-level DEFAULTs that only applied to server.py's own CREATE TABLE --
        # when honeychain_db.py's stricter NOT NULL, no-default hives table was
        # created first, omitting these two columns crashed startup every time).
        # INSERT OR IGNORE (not plain INSERT): server.py runs with workers=2, so two
        # worker processes both run this lifespan concurrently on a fresh database --
        # both see count==0 and both attempt this same insert, and a plain INSERT
        # crashes the losing worker with "UNIQUE constraint failed: hives.hive_id".
        # Uvicorn does respawn the crashed worker, which then sees count==100 and
        # skips seeding, so it self-heals -- but it's a noisy, unnecessary crash on
        # every fresh start. IGNORE makes the second worker's redundant insert a
        # harmless no-op instead.
        cursor.executemany("""
        INSERT OR IGNORE INTO hives (hive_id, apiary_id, beekeeper_id, name, location, queen_age_months, installation_date, tare_weight_kg, status, last_seen_epoch, last_health_score)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
        """, hive_rows)
...
async def lifespan(app: FastAPI):
    # honeychain_db.init_db() must run FIRST: it creates the real, FK-constrained
    # `hives` schema (and clusters/beekeepers/apiaries, which init_database()'s own
    # demo-hive seeding depends on). Both functions use CREATE TABLE IF NOT EXISTS
    # for `hives`, so whichever runs first wins the schema for the life of the
    # database file -- running init_database() first (the old order) let its
    # weaker, FK-less legacy schema silently win on a fresh DB, and left
    # clusters/beekeepers/apiaries nonexistent when init_database() tried to seed
    # a demo beekeeper into them, breaking the on-chain harvest lookup below.
